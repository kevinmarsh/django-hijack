...
class TestHijackUserAdminMixin:
    def test_user_admin(self, admin_client, bob):
        url = reverse("admin:test_app_customuser_changelist")
        response = admin_client.get(url)
        assert response.status_code == 200
        button = (
            '<button type="submit" name="hijack_user" '
            f'value="{bob.pk}" class="button">HIJACK</button>'
        )
        assert button.encode() in response.content
